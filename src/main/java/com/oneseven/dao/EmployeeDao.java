...
public class EmployeeDao {
	...
	public Employee SearchEmployee(int id) throws Exception {
		...
		try {
			conn = ConnectionDatabase.Instance().getConnection();
			// Enviando Consulta
			callableStatement = conn.prepareCall("SELECT EmployeeId,Name,LastName,Age,PositionID,PositionName"
					+ " FROM Employee INNER JOIN Position ON Position_Id = PositionID WHERE EmployeeId = ?");
